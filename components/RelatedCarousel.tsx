...
export default function RelatedCarousel({ slugs }: RelatedCarouselProps) {
  ...
  return (
    <div className="px-6 py-4 border-t border-[var(--color-border)] bg-[var(--color-surface)] flex-shrink-0 transition-colors duration-300">
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-[9px] font-bold tracking-[0.12em] text-[#f48c25] mb-0.5">NAVIGATION</p>
          <p className="text-[14px] font-bold text-[var(--color-text)]">Related Items</p>
        </div>
        <div className="flex gap-1.5">
          <button
            onClick={() => scroll('left')}
            className="w-7 h-7 rounded-lg bg-[var(--color-surface-2)] border border-[var(--color-border)] text-[var(--color-text-2)] flex items-center justify-center hover:border-[#f48c25] hover:text-[#f48c25] transition-all"
          >
      ...
      <div ref={scrollRef} className="flex gap-3 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
        {related.map((item) => (
          ...
          >
            <div
              className="w-[110px] h-20 rounded-lg border border-[var(--color-border)] flex items-center justify-center hover:border-[#f48c25] transition-colors"
            ...
            <p className="text-[11px] font-semibold text-[var(--color-text)] truncate">{item.name}</p>
