  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex justify-between items-center mb-1">
        <p className="text-[12px] font-bold text-[var(--color-text)]">Vitamins &amp; Minerals</p>
        <p className="text-[9px] font-semibold tracking-widest text-[var(--color-text-3)]">% DAILY VALUE</p>
      </div>
      {vitamins.map((v, i) => (
        <div key={i} className="flex items-center gap-2.5">
          <p className="text-[11px] text-[var(--color-text-2)] w-[90px] flex-shrink-0">{v.name}</p>
          <div className="flex-1 h-1 bg-[var(--color-surface-3)] rounded-full overflow-hidden">
            <div
